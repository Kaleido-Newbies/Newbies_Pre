...
public class Ghost : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        _ghostStat = GetComponent<GhostStat>();
        isAttacking = false;
        isTracking = false;
        stopTracking = false;
    ...
    void move()
    {
        ...
        Debug.DrawRay(fontVec, Vector3.down * 3, new Color(0, 1, 0));
        RaycastHit2D rayHit = Physics2D.Raycast(fontVec, Vector2.down, 3, LayerMask.GetMask("Floor"));
    ...
    void Think()
    {
     ...
     public void onDamaged(Vector2 targetPos)
     {
         ...
         _ghostStat.Hit(PlayerStat._playerStat.atk);
     }
    ...
    // 자신이 바라보는 방향의 일정거리 안에 있는 플레이어를 추적하는 함수
     void trackPlayer()
     {
         ...
         if (rayHit.collider)
         {
             ...
             if(nextMove == -1)
                transform.position = new Vector3(rayHit.collider.gameObject.transform.position.x + 1.8f , transform.position.y, 0);
             else
                 transform.position = new Vector3(rayHit.collider.gameObject.transform.position.x + -1.8f , transform.position.y, 0);
     ...
     private void OnDrawGizmos()
     {
         ...
         Gizmos.DrawWireCube(new Vector3(transform.position.x + 1.8f, transform.position.y, transform.position.z),
             boxSize);
         Gizmos.DrawWireCube(new Vector3(transform.position.x - 1.8f, transform.position.y, transform.position.z),
             boxSize);
     }
     ...
     void readyToAttack()
     {
         if (normalCurTime <= 0)
         {
             ...
             if (spriterenderer.flipX)
                 collider2Ds = Physics2D.OverlapBoxAll(new Vector2(transform.position.x - 0.4f, transform.position.y), boxSize, 0);
             else
                 collider2Ds = Physics2D.OverlapBoxAll(new Vector2(transform.position.x + 0.4f, transform.position.y), boxSize, 0);
             ...
             foreach (Collider2D collider in collider2Ds)
             {
                 if (collider.gameObject.layer == 7)
                 {
                     ...
                     StartCoroutine(setAttacktriger());
                     Invoke("Attack", 1f);
                 }
             }
         }
